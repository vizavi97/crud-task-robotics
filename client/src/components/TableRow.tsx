...
import {showDeleteModal, showUpdateModal} from "../store/action/user.action";
...
export const TableRow: React.FC<TableRowInterface> = ({
    ...
    const deleteHandler = (id: string) => {
        localStorage.setItem("removingId", id)
        dispatch(showDeleteModal())

    }
    return (
        <tr className={isClosed ? "content-table__row close" : "content-table__row"}>
            <td className={"content-table__name"}>
                <span className={'content-table__info'}>№ ФИО пользователя</span>
                <span>{number + " " + surname + " " + name[0].toUpperCase() + "." + middleName[0].toUpperCase() + "."}</span>
            </td>
            <td className={"content-table__role"}>
                <span className={'content-table__info'}>Роль</span>
                <span>{role.title}</span>
            </td>
            <td className={"content-table__birthday"}>
                <span className={'content-table__info'}>Дата рождения</span>
                <span>{convertDate(birthday)}</span>
            </td>
            <td className={"content-table__place"}>
                <span className={'content-table__info'}>Место рождения</span>
                <span>{birthPlace}</span>
            </td>
            <td className={"content-table__email"}>
                <span className={'content-table__info'}>Почта</span>
                <span>{email}</span>
            </td>
            <td className={"content-table__phone"}>
                <span className={'content-table__info'}>Телефон</span>
                <span>{phoneNumber}</span>
            </td>
            <td className={"content-table__register"}>
                <span className={'content-table__info'}>Регистрации</span>
                <span>{convertDate(registerDate)}</span>
            </td>
            <td className={"content-table__update"}>
                <span className={'content-table__info'}>Изменение</span>
                <span>{convertDate(lastUpdate, true)}</span>
            </td>
            <td className={"content-table__change"}>
                <button type={"button"} onClick={() => dispatch(showUpdateModal(id))}>
            ...
            <td className={"content-table__arrow"} onClick={() => setIsClosed(state => !state)}>
                <svg width="11" height="7" viewBox="0 0 11 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1.5 -4.15258e-07L-6.55671e-08 1.5L5.5 7L11 1.5L9.5 -6.55671e-08L5.5 4L1.5 -4.15258e-07Z"
                          fill="#B7BAC6"/>
                </svg>
            </td>
        </tr>
